    ...
    <SiteLayout>
      <section className="border-b border-border bg-secondary/40">
        <div className="container-page py-20">
          <div className="eyebrow"><span className="h-px w-8 bg-brand-green" /> {eyebrow}</div>
          <h1 className="mt-3 max-w-3xl font-display text-4xl font-bold text-navy sm:text-5xl">{title}</h1>
      ...
      <section className="container-page py-20">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr]">
          ...
          {stats && (
            <aside className="self-start rounded-2xl bg-gradient-hero p-8 text-navy-foreground shadow-elegant">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-green">By the numbers</div>
        ...
        <div className="mt-16 flex flex-col items-start justify-between gap-6 rounded-2xl border border-border bg-card p-10 shadow-card lg:flex-row lg:items-center">
          ...
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-md bg-navy px-6 py-3 text-sm font-semibold text-navy-foreground transition hover:bg-navy-deep">
            Contact a Supplier <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
    </SiteLayout>
